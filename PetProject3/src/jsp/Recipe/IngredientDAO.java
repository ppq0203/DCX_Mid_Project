...
public class IngredientDAO {
    ...
    public void saveToCsv(String id) throws SQLException, IOException {
		String filePath="C:\\Users\\anyware\\Desktop\\밥사조 프로젝트\\test.csv";
		String tableName = "Ingredients";
			
		// id에 등록된 식재료를 가져오기 위한 쿼리문 생성
		String sql = " SELECT ING_NAME FROM " + tableName + " WHERE id = ? ";
		pstmt = conn.prepareStatement(sql);
	    pstmt.setString(1, id);
	    rs = pstmt.executeQuery();
		...
		while(rs.next()){
			...
			ingredientsList.add(ING_NAME);
						}		
			
			// 추천 레시피를 위한 쿼리문 생성
			String ingredientStr = String.join("', '", ingredientsList);
//    		String tableName = "Recipe_Content";
			sql = "SELECT r.CKG_NM, ri.ING_NAME, ri.ING_AMOUNT, ri.ING_UNIT, r.CKG_KND_ACTO_NM, r.CKG_TIME_NM " + 
    		"FROM Recipe r " + 
    		"INNER JOIN Recipe_Ingredients ri ON r.RCP_SNO = ri.RCP_SNO " + 
    		"INNER JOIN Ingredients i ON ri.ING_NAME = i.ING_NAME " +    		
    		"WHERE i.ING_NAME IN ('" + ingredientStr + "') " + 
    		"GROUP BY r.CKG_NM,ri.ING_NAME,ri.ING_AMOUNT, ri.ING_UNIT, r.CKG_KND_ACTO_NM, r.CKG_TIME_NM " + 
			...
			fw.append("레시피,재료,종류,조리시간\n");
			
		while (rs.next()){
			String CKG_NM = rs.getString("CKG_NM");
			String ING_NAME = rs.getString("ING_NAME");
			String ING_AMOUNT = rs.getString("ING_AMOUNT");
			String ING_UNIT = rs.getString("ING_UNIT");
			String CKG_KND_ACTO_NM = rs.getString("CKG_KND_ACTO_NM");
			String CKG_TIME_NM = rs.getString("CKG_TIME_NM");
			
			String Ingredients = ING_NAME + " " + ING_AMOUNT + " " + ING_UNIT;
			fw.append(CKG_NM).append(",");
			fw.append(Ingredients).append(",");
			fw.append(CKG_KND_ACTO_NM).append(",").append(CKG_TIME_NM).append(",").append("\n");
						 }
			fw.flush();
			fw.close();
    ...
    public void saveToCsv1(String tableName) throws SQLException, IOException {
    	String filePath="C:\\Users\\anyware\\Desktop\\밥사조 프로젝트\\test.csv";
    	String sql = "SELECT * FROM ? where ING_INFO = '마늘' ";
    	PreparedStatement pstmt = conn.prepareStatement(sql);
    	...
    	System.out.println(Arrays.deepToString(dataList.toArray(new String[0][0])));
